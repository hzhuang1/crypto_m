...
extern void single_02(XXH64_JOB **job_vecs, int job_cnt, int blk_cnt, void *buf);
...
printf("%s, %d, v1:0x%lx, v2:0x%lx, v3:0x%lx, v4:0x%lx\n", __func__, __LINE__, job_vec[0]->digest[0], job_vec[0]->digest[1], job_vec[0]->digest[2], job_vec[0]->digest[3]);
	printf("ASM version:\n");
	// SVE512 / 64bit = 8 (lanes)
	memset(buf, 0, 2 * (seed_size + XXH64_BLOCK_SIZE));
	single_01(job_vec, 1, 1, buf);
	//dump_buf(buf, XXH64_MAX_LANES * (seed_size + XXH64_BLOCK_SIZE));
	dump_buf(buf, 1 * (seed_size + XXH64_BLOCK_SIZE));
	printf("C version:\n");
	{
		for (i = 0; i < XXH64_MAX_LANES; i++) {
